...
//		request.setCharacterEncoding("utf-8");
		...
		String saveId = request.getParameter("saveId"); // "on" 아니면 null이 올 것
		...
		// false 존재하면 해당 세션 객체를, 존재하지 않으면 null 리턴
		HttpSession session = request.getSession();
//		System.out.println(session.getId()); // JSESSIONID 동일 필터생성했으니 주석
		...
		if(member != null && password.equals(member.getPassword())) {
			...
			if(saveId != null) {
				// 만약 max-age설정이 없다면, 세션쿠키로 등록. 브라우져 종료시 폐기
				// 아래 작성한 것처럼 max-age설정이 있다면, 영속쿠키로 등록. 지정한 시각에 폐기
				cookie.setMaxAge(7 * 24 * 60 * 60); // 일주일동안 관리 - 초단위 일주일후 폐기
			}
			else {
				cookie.setMaxAge(0); // 0은 즉시삭제를 의미
			}
